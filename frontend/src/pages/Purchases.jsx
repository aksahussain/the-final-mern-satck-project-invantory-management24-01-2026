...
import { Plus, FileText, Check, X, Trash2, Search } from 'lucide-react';
...
const Purchases = () => {
    ...
    const [formData, setFormData] = useState({
        supplier: '',
        products: [{ product: '', quantity: 1, costPrice: 0 }],
    });
    ...
    const handleCreateOrder = async (e) => {
        ...
        try {
            ...
            const totalAmount = validProducts.reduce((acc, item) => acc + (item.quantity * item.costPrice), 0);
            await API.post('/orders/purchase', { ...formData, products: validProducts, totalAmount });
            setIsModalOpen(false);
            fetchData();
            setFormData({ supplier: '', products: [{ product: '', quantity: 1, costPrice: 0 }] });
        } catch (error) {
    ...
    const addProductRow = () => {
        setFormData({
            ...formData,
            products: [...formData.products, { product: '', quantity: 1, costPrice: 0 }]
        });
    };
    ...
    const handleActivateOrder = async (id) => {
        if (!window.confirm('Mark this order as received and update stock?')) return;
        try {
            await API.put(`/orders/purchase/status/${id}`, { status: 'completed' });
            alert('Order activated successfully! Stock has been updated.');
            navigate('/dashboard'); // Take user to dashboard to see the updated stats
        } catch (error) {
            console.error("Activation failed:", error);
            const errorMsg = error.response?.data?.message || error.message || 'Failed to activate order';
            alert(errorMsg);
        }
    };
    ...
    const handleProductChange = (index, field, value) => {
        ...
        // Auto-set cost price if product selected
        if (field === 'product') {
            const prod = products.find(p => p._id === value);
            if (prod) newProducts[index].costPrice = prod.price;
        }
    ...
    const downloadInvoice = async (id) => {
        try {
            const response = await API.get(`/orders/invoice/purchase/${id}`, { responseType: 'blob' });
    ...
    const filteredOrders = orders.filter((order) => {
        ...
        return matchesDate && (!keyword || searchableText.includes(keyword));
    });
    ...
    const downloadFilteredReport = async () => {
        try {
            ...
            const response = await API.get(`/orders/purchase/report/pdf?${params.toString()}`, { responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `purchase-orders-${new Date().toISOString().slice(0, 10)}.pdf`);
            document.body.appendChild(link);
            link.click();
            link.remove();
            ...
            alert('Failed to download purchase report');
        } finally {
        ...
        <div className="space-y-6">
            <div className="flex flex-wrap justify-between items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400">Manage stock replenishment</p>
                </div>
                ...
                <div className="p-4 border-b border-gray-100 dark:border-gray-700">
                    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto]">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Search supplier, product, status, or order ID"
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex space-x-3">
                                        ...
                                        {['admin', 'inventory_manager', 'inventory_managerr', 'manager'].includes(user?.role?.toLowerCase()?.trim()) && order.status === 'pending' && (
                                            <button
                                                onClick={() => handleActivateOrder(order._id)}
                                                className="text-green-600 hover:text-green-800 dark:text-green-400"
                                                title="Activate Order (Update Stock)"
                                            >
                                                <Check size={18} />
                                            </button>
                                        )}
            ...
            <AnimatePresence>
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                        ...
                        >
                            <h3 className="text-xl font-bold mb-6 dark:text-white">Create Purchase Order</h3>
                            <form onSubmit={handleCreateOrder} className="space-y-4">
                                ...
                                <div className="border-t dark:border-gray-700 pt-4 mt-4">
                                    <div className="flex justify-between items-center text-lg font-bold dark:text-white">
                                        ...
                                        <span>₹{formData.products.reduce((acc, item) => acc + (item.quantity * (item.costPrice || 0)), 0).toLocaleString()}</span>
                                    </div>
                                </div>
